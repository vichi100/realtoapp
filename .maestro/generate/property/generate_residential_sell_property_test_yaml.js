...
function transformTestData(parsedData) {
  return {
    ...
    property_details_parking_type_lower: parsedData.tapOn.property_details_parking_type.toLowerCase(),
    ...
    property_details_lift_lower: parsedData.tapOn.property_details_lift.toLowerCase(),
...
function createTestDataVariations(baseData) {
  return [
    ...
    //   sell_details_expected_sell_price: '30000000'
    // },
    // { 
    //   ...baseData, 
    //   variation: '3',
...
try {
  ...
  testDataVariations.forEach((testData, index) => {
    const outputFilePath = path.join(__dirname, `../../ios/property/add_residential_sell_test_case_${index + 1}.yaml`);
    const yamlContent = generateYaml(template, testData);
    fs.writeFileSync(outputFilePath, yamlContent);
    console.log(`Generated sell test case ${index + 1} at: ${outputFilePath}`);
  });
