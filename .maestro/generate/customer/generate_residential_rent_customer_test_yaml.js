...
function transformTestData(parsedData) {
  return {
    ...
    locality_details_required_for_lower: parsedData.tapOn.locaity_details_requied_for.toLowerCase(),
    ...
    final_details_max_deposit_tag_value: parsedData.assertive.final_details_max_deposit_tag_value,
    ...
    final_details_lift_mandatory_tag: parsedData.assertive.final_details_lift_mandatory_tag,
    final_details_tenant_type_tag: parsedData.assertive.final_details_tenant_type_tag
  };
}
...
function createTestDataVariations(baseData) {
  return [
    ...
    { ...baseData, 
      variation: '2',
      customer_details_name: 'Customer Rent Residential 2',
      customer_details_mobile: '9933055552',
      customer_details_address: 'Juhu Mumbai',
      final_details_name_header: '.*Customer Rent Residential 2.*',
      final_details_mobile_header: '.*9933055552.*',
    },
...
const dataFileLocation = '../../dataset/customer/residential_rent_customer_data.txt'
const templateFileLocation = '../../ios/template/customer/add_residential_rent_customer_template.yaml';
...
try {
  ...
  testDataVariations.forEach((testData, index) => {
    const outputFilePath = path.join(__dirname, `../../ios/customer/add_residential_rent_customer_test_case_${index + 1}.yaml`);
    const yamlContent = generateYaml(template, testData);
    fs.writeFileSync(outputFilePath, yamlContent);
    console.log(`Generated residential rent customer test case ${index + 1} at: ${outputFilePath}`);
  });
